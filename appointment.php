<?php
// Database connection
$conn = new mysqli("localhost", "root", "", "pets_vibe");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $serviceType = $_POST['serviceType']; // New service type field
    ...
    if ($uploadOk == 1) {
        if (move_uploaded_file($_FILES["petPicture"]["tmp_name"], $targetFile)) {
            ...
            $stmt = $conn->prepare("INSERT INTO appointments (petName, ownerName, contactNumber, appointmentDate, petPicture, email, address, serviceType) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssssss", $petName, $ownerName, $contactNumber, $appointmentDate, $targetFile, $email, $address, $serviceType);
